...
public class AnnotationHover implements IAnnotationHover
    {
        ...
        public String getHoverInfo(ISourceViewer sourceViewer, int lineNumber)
        {
            ...
            for (int i = 0; i < messages.length; i++)
            {
                ...
                if (i < messages.length - 1) buffer.append(System.getProperty("line.separator")); //$NON-NLS-1$
            }
            ...
            while (iter.hasNext())
            {
                ...
                if (object instanceof MarkerAnnotation)
                {
                    ...
                    if (compareRulerLine(model.getPosition(annotation), document, line))
                    {
                        ...
                        if (message != null && message.trim().length() > 0) messages.add(message);
                    }
                }
            }
        ...
        private boolean compareRulerLine(Position position, IDocument document, int line)
        {
            ...
            try
            {
                if (position.getOffset() > -1 && position.getLength() > -1) { return document
